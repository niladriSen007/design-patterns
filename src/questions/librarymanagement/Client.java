package questions.librarymanagement;

import questions.librarymanagement.entity.BookOrMagazineCopy;
import questions.librarymanagement.entity.ItemType;
import questions.librarymanagement.facade.LibraryManagement;
import questions.librarymanagement.observer.Member;
import questions.librarymanagement.strategy.SearchByAuthorStrategy;
import questions.librarymanagement.strategy.SearchByTitleStrategy;
...
public class Client {
    static void main() {
        ...
        List<BookOrMagazineCopy> hobbitCopies = library.addItem(ItemType.BOOK, "B001", "The Hobbit", "J.R.R. Tolkien", 2);
        List<BookOrMagazineCopy> duneCopies = library.addItem(ItemType.BOOK, "B002", "Dune", "Frank Herbert", 1);
        List<BookOrMagazineCopy> natGeoCopies = library.addItem(ItemType.MAGAZINE, "M001", "National Geographic", "NatGeo Society", 3);
        ...
        library.checkout(alice.getMemberId(), hobbitCopies.get(0).getId()); // Alice checks out The Hobbit copy 1
        library.checkout(bob.getMemberId(), duneCopies.get(0).getId()); // Bob checks out Dune copy 1
        library.printCatalog();

        System.out.println("Attempting to checkout an already checked-out book:");
        library.checkout(charlie.getMemberId(), hobbitCopies.get(0).getId()); // Charlie fails to check out The Hobbit copy 1
        ...
        library.placeHold(charlie.getMemberId(), "B002"); // Charlie places a hold on Dune
        ...
        library.checkout(charlie.getMemberId(), duneCopies.get(0).getId());

        System.out.println("\nTrying to check out the same on-hold item by another member (Alice):");
        library.checkout(alice.getMemberId(), duneCopies.get(0).getId()); // Alice fails, it's checked out by Charlie now.
